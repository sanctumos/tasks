...
export function ComposerPasteManager(options) {
...
ComposerPasteManager.prototype.remove = function (id) {
  this.pending = this.pending.filter(function (a) {
    return a.id !== id;
  });
  this._renderChips();
  this.onChange();
...
ComposerPasteManager.prototype.renderBubbleHtml = function (payload) {
  ...
  (payload.attachments || []).forEach(function (a) {
    html +=
      '<div class="sanctum-composer-bubble-attach" data-att-id="' +
      escapeHtml(a.id) +
      '">' +
    ...
    if (a.text) {
      html +=
        '<button type="button" class="sanctum-composer-preview-btn" data-preview-text="' +
        escapeHtml(a.text) +
        '" data-preview-name="' +
        escapeHtml(a.filename) +
        '">Preview</button>';
...
export const AskQComposerUtils = { formatBytes, escapeHtml };

if (typeof window !== 'undefined') {
  window.ComposerPasteManager = ComposerPasteManager;
  window.AskQComposerUtils = AskQComposerUtils;
}

export default { ComposerPasteManager, AskQComposerUtils };
