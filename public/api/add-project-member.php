...
$role = isset($body['role']) ? (string)$body['role'] : 'member';

$result = addProjectMember((int)$user['id'], $projectId, $targetUserId, $role);
if (!$result['success']) {
    apiError('project.member_failed', $result['error'] ?? 'Could not add member', 400);
}
