...
Q gets **about 15 board tools** — enough to read and write tasks, comments, and documents for projects the chatter can already see. Q does **not** get admin tools (users, API keys, audit), bulk mutators, or org provisioning. Full SDK stays in the CLI; this table is **exposure** for Letta attach and future `describe-profile chatter`.

---

## Profile: `chatter` (15 tools)
...
|----------|------------------------|--------|
| **Admin / IAM** | `create-user`, `disable-user`, `reset-user-password`, `create-api-key`, `revoke-api-key`, `list-api-keys`, `list-users`, `search-users`, `list-audit-logs` | Chatter lane; escalation = human admin |
...
Full catalog today: **49** commands (`tasks/cli.py --describe`). Chatter = **15** (~69% reduction in schema surface).
...
2. Attach **only** the 15 tools above (+ future `sanctum__tools` governor when Phase 2b ships).
